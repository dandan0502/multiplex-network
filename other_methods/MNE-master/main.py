...
def train_LINE_model(edges, filename):
    preparation_command = 'LD_LIBRARY_PATH=/usr/local/lib\nexport LD_LIBRARY_PATH'
    os.system('python ../OpenNE-master/src/main.py\
              --method line --input {}\
...
for i in range(number_of_groups):
    ...
    for edge_type in edge_data_by_type_by_group:
        ...
        for j in range(number_of_groups):
            if j == i:
                # print(edge_data_by_type_by_group[edge_type][j])
    ...
    for edge_type in training_data_by_type:
        ...
        filename = LINE_path + str(edge_type) + '.edges'
        LINE_model = train_LINE_model(training_data_by_type[edge_type], filename)
