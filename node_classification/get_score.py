...
def get_Ci1(email_layer, hist_layer, email_graph, hist_graph):
    ...
    email_layer = email_layer[email_layer['From'] != email_layer['To']]
    ...
    for key, value in email_oi_d.items():
        ...
        for row in key_row.values.tolist():
            a = hist_layer[(hist_layer['From'] == row[1])]
            if not a.empty:
    ...
    for key, value in hist_oi_d.items():
        ...
        for row in key_row.values.tolist():
            a = email_layer[(email_layer['From'] == row[1])]
            if not a.empty:
...
def get_property_score(G, Conv_G, df, Conv_df, node):
    ...
    nbr_strength = get_si(df)
    entropy_degree = get_cross_entropy(G, Conv_G, overlap_degree)
    first_coef = get_Ci1(df, Conv_df, G, Conv_G)
    inter_dependence = get_lambdai(df, Conv_df, G, Conv_G)
    # print(nbr_strength, entropy_degree, first_coef, inter_dependence)
    return zscore_norm_overlap_degree, nbr_strength, entropy_degree, first_coef, inter_dependence
